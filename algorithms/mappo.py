"""
MAPPO — Multi-Agent Proximal Policy Optimization (untrained baseline).

Architecture: shared actor-critic with centralized value function.
Each agent observes local neighborhood (K nearest neighbors' relative
positions, headings, speeds) and outputs a speed action.

This is an UNTRAINED policy (random initialization) — serves as a
baseline to measure architecture overhead and show what RL performance
looks like before training. Your colleague can train this with PPO.

Complexity: O(N·K·d) per step — K neighbors, d = hidden dimension.
"""

import numpy as np
from .base import BaseAlgorithm
import config as cfg

# observation: [own_speed, own_progress_frac, K * (rel_x, rel_y, rel_speed, heading_dot)]
K_NEIGHBORS = 10
OBS_DIM = 2 + K_NEIGHBORS * 4  # 42
HIDDEN_DIM = 64


class SimpleMLPPolicy:
    """Two-layer MLP policy: obs -> hidden -> speed."""

    def __init__(self, obs_dim: int, hidden_dim: int, seed: int = 42):
        rng = np.random.RandomState(seed)
        # Xavier initialization
        self.w1 = rng.randn(obs_dim, hidden_dim) * np.sqrt(2.0 / obs_dim)
        self.b1 = np.zeros(hidden_dim)
        self.w2 = rng.randn(hidden_dim, 1) * np.sqrt(2.0 / hidden_dim)
        self.b2 = np.zeros(1)

    def forward(self, obs: np.ndarray) -> float:
        """obs: (obs_dim,) -> scalar speed in [MIN_SPEED, MAX_SPEED]."""
        h = np.tanh(obs @ self.w1 + self.b1)
        out = h @ self.w2 + self.b2
        # sigmoid to [0, 1] then scale to speed range
        sig = 1.0 / (1.0 + np.exp(-np.clip(out[0], -10, 10)))
        return cfg.MIN_SPEED + sig * (cfg.MAX_SPEED - cfg.MIN_SPEED)

    def forward_batch(self, obs_batch: np.ndarray) -> np.ndarray:
        """obs_batch: (N, obs_dim) -> (N,) speeds."""
        h = np.tanh(obs_batch @ self.w1 + self.b1)
        out = (h @ self.w2 + self.b2).ravel()
        sig = 1.0 / (1.0 + np.exp(-np.clip(out, -10, 10)))
        return cfg.MIN_SPEED + sig * (cfg.MAX_SPEED - cfg.MIN_SPEED)


class MAPPO(BaseAlgorithm):
    name = "MAPPO"
    complexity = "O(N*K*d)"

    def __init__(self):
        self.policy = SimpleMLPPolicy(OBS_DIM, HIDDEN_DIM)

    def _build_observations(self, positions, headings, speeds, preferred, active):
        """Build local observations for each agent."""
        from scipy.spatial import cKDTree

        n = len(positions)
        obs = np.zeros((n, OBS_DIM))

        active_idx = np.where(active)[0]
        if len(active_idx) < 1:
            return obs

        pos_a = positions[active_idx]
        tree = cKDTree(pos_a) if len(active_idx) > 1 else None

        for ii, i in enumerate(active_idx):
            # own features
            obs[i, 0] = speeds[i] / cfg.MAX_SPEED
            obs[i, 1] = speeds[i] / preferred[i] if preferred[i] > 0 else 0

            if tree is None or len(active_idx) < 2:
                continue

            # K nearest neighbors
            k = min(K_NEIGHBORS, len(active_idx) - 1)
            dists, idxs = tree.query(pos_a[ii], k=k + 1)

            slot = 0
            for nn_idx, nn_dist in zip(idxs[1:], dists[1:]):
                if slot >= K_NEIGHBORS:
                    break
                j = active_idx[nn_idx]
                rel = positions[j] - positions[i]
                base = 2 + slot * 4
                obs[i, base] = rel[0] / cfg.WORLD_SIZE
                obs[i, base + 1] = rel[1] / cfg.WORLD_SIZE
                obs[i, base + 2] = speeds[j] / cfg.MAX_SPEED
                obs[i, base + 3] = np.dot(headings[i], headings[j])
                slot += 1

        return obs

    def compute_speeds(self, positions, headings, speeds, preferred, active, dt):
        obs = self._build_observations(positions, headings, speeds, preferred, active)
        cmd = self.policy.forward_batch(obs)
        cmd[~active] = 0.0
        return cmd
